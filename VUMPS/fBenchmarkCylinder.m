...
filepath = GetUniqueFilePath([fullfile(statefldr,name),'.mat']);
save(filepath,'AL','AR','AC','C','W');
disp(['saved under ',filepath]);
%% post edits
...
if save_plots
    ...
    %         ctn = ctn + 1;
    %     end
    filepath = GetUniqueFilePath([fullfile(fldr,name),'.fig'])
    savefig(fh,filepath);
    disp(['saved as ',filepath]);
end
