function [AL,AR,AC,C,stats] = fVUMPS_MPO(W,paramin)
%% load parameters
...
prec = max(max(abs(cell2mat(tmpg))));
...
if trueLR
    L = fMPSTMeig(AR,'l',0,C'*C,[],'lr');
    R = fMPSTMeig(AL,'r',0,C*C',[],'lr');
else
    L = C'*C;
    R = C*C';
end
...
XL = fMPO_TMeig(AL,W,[],R,'l',InvEthresh,max(InvEthresh,tol/10),[],false);
XR = fMPO_TMeig(AR,W,L,[],'r',InvEthresh,max(InvEthresh,tol/10),[],false);
...
%     prec = max(errs);
    ...
    XL = fMPO_TMeig(AL,W,[],R,'l',InvEthresh,max(InvEthresh,tol/10),XL,false);
    XR = fMPO_TMeig(AR,W,L,[],'r',InvEthresh,max(InvEthresh,tol/10),XR,false);
